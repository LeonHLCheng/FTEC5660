...
1. Each receipt image is first encoded into Base64, but instead of embedding the Base64 directly into the prompt, which causes the LLM to treat the encoded bytes as text and misinterpret Base64 image data cause hallucinations of the result. Therefore We explicitly separate the prompt into a text block and an `image_url` block so the model correctly interprets the image.
